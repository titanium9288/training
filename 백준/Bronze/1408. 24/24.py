...
def time_to_sec(f_time: str) -> int:
    ...
    time_in_sec = f_hour * 3600 + f_min * 60 + f_sec
...
def sec_to_time(sec: int) -> str:
    ...
    # 시간을 문자열로 변환
    time_str = f"{f_hour:02d}:{f_min:02d}:{f_sec:02d}"
    return time_str
...
current_time = input()
current_time_in_sec = time_to_sec(current_time)
...
# 초로 변환한 시간을 문자열로 변환
remain_time_in_sec = start_time_in_sec - current_time_in_sec
remain_time_str = sec_to_time(remain_time_in_sec)

# 시간 출력
print(sec_to_time(remain_time_in_sec))
